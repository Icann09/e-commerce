...
export default function ColorSwatches({
  ...
  const setColorIndex = useVariantStore((s) => s.setColorIndex);

  const selected = useVariantStore(
    ...
    >
      {variants.map((v, i) => {
        ...
        return (
          <button
            key={`${v.color}-${i}`}
            onClick={() => setColorIndex(productId, i)}
            aria-label={`Color ${v.color}`}
            aria-selected={isActive}
            role="option"
            className={`relative h-[72px] w-[120px] overflow-hidden rounded-lg ring-1 ring-light-300 transition
              focus:outline-none focus-visible:ring-2 focus-visible:ring-[--color-dark-500]
